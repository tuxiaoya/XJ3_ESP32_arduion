...
private: //  成员变量  小写加下划线  私有方法  + 私有
  ...
  void TXD_GETpar_Handshake(uint8_t HWSGAddress = 0); // 0-15+0xC0  连续发两次  命令HWSG送出工作参数  DN
  void TXD_SETpar_Handshake(uint8_t HWSGAddress = 0); // 0-15+0xC0  连续发两次  命令HWSG收工作参数  DN
